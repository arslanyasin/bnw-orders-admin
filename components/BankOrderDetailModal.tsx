...
interface BankOrderDetailModalProps {
  isOpen: boolean;
  onClose: () => void;
  order: BankOrder | null;
  onAddComment: (orderId: string, comment: string) => Promise<void>;
  isAddingComment?: boolean;
}

const BankOrderDetailModal: React.FC<BankOrderDetailModalProps> = ({
  isOpen,
  onClose,
  order,
  onAddComment,
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Bank Order Details" size="lg">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-red-600 font-semibold">{Math.abs(order.redeemedPoints).toLocaleString()}</p>
            </div>
          </div>
        </div>
...
export default BankOrderDetailModal;
