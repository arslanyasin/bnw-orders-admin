...
const BankOrdersPage = () => {
  ...
  const [searchField, setSearchField] = useState<'all' | 'poNumber' | 'productId' | 'product'>('all');
  ...
  const [selectedOrderForPO, setSelectedOrderForPO] = useState<BankOrder | null>(null);
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await bankOrderService.getAll(params);

      console.log('[Bank Orders] Response:', {
      ...
      console.error('[Bank Orders] Failed to fetch orders:', error);
      setOrders([]);
      setTotalPages(1);
      setTotalRecords(0);
    } finally {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await bankOrderService.delete(id);
      fetchOrders();
    } catch (error: any) {
  ...
  const handleStatusUpdate = async (id: string, status: OrderStatus) => {
    try {
      await bankOrderService.updateStatus(id, status);
      fetchOrders();
    } catch (error: any) {
  ...
  const handleImport = async (bankId: string, file: File) => {
    try {
      setIsImporting(true);
      console.log('Importing orders:', { bankId, fileName: file.name });

      const response = await bankOrderService.import(bankId, file);
      ...
      if (response.data) {
        ...
        if (failedCount > 0) {
          ...
          alert(`Successfully imported ${successCount} order${successCount !== 1 ? 's' : ''}!`);
        }
      } else {
      ...
      alert(error.message || 'Failed to import orders');
    } finally {
      setIsImporting(false);
    }
  };

  const handleCreatePOFromOrder = (order: BankOrder) => {
  ...
  const handleDispatchOrder = (order: BankOrder) => {
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col md:flex-row gap-4">
                <div className="flex-1 relative">
                  ...
                  <input
                    type="text"
                    placeholder="Search orders by PO Number, Product ID, or Product Name..."
